...
class RegistrationController extends AbstractController
{
    ...
    public function register(Request $request, UserPasswordHasherInterface $userPasswordHasher, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->emailVerifier->sendEmailConfirmation('app_verify_email', $user,
                (new TemplatedEmail())
                    ->from(new Address('homeantinuisible@gmail.com', 'Abdenour Maali'))
                    ->to($user->getEmail())
                    ->subject('Confirmation de l\'enregistrement du compte.')
                    ->htmlTemplate('registration/confirmation_email.html.twig')
            );
            // do anything else you need here, like send an email

            return $this->redirectToRoute('registration_waiting_for_email_confirmation');
        }
    ...
    #[Route('/register/waiting-for-email-confirmation', name: 'registration_waiting_for_email_confirmation', methods:['GET'])]
